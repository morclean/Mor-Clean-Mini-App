...
const startOfWeek = () => { const d=new Date(); const day=d.getDay(); const diff=d.getDate()-day+(day===0?-6:1); return new Date(d.setDate(diff)).toISOString().slice(0,10); };
...
// strings like POWVHCWKB… etc
const looksLikeWeirdId = (s) => !!s && /^[A-Z0-9]{10,}$/.test(s);

// pick safe labels
const titleLine = (job) => {
  // main headline = client if present, else a clean name
  return job.client?.trim() || job.title?.trim() || "Cleaning";
};
const subline = (job) => {
  // show a human “service” label (hide weird IDs), plus time
  const t = looksLikeWeirdId(job.title) ? "Cleaning" : (job.title || "Cleaning");
  const time = job.start ? `${job.start}${job.end?`–${job.end}`:""}` : "";
  const date = fmtDate(job.date);
...
function CleanerView() {
    ...
    (async () => {
      try {
        ...
        const mapped = events.map((e) => ({
          id: `${e.date}-${(e.client||"").slice(0,40)}-${(e.title||"Clean").slice(0,40)}`,
          date: e.date,
          start: e.start || "",
          end: e.end || "",
          title: e.title || "Clean",
  ...
  const toggleJob  = id => setJobOpen(p => ({...p, [id]:!p[id]}));
  const toggleRoom = (jid,room) => setRoomOpen(p => ({...p,[jid]:{...p[jid],[room]:!p[jid]?.[room]}}));
  const toggleTask = (jid, room, task) =>
    setChecked(p => { const n=structuredClone(p); n[jid]??={}; n[jid][room]??={}; n[jid][room][task]=!n[jid][room][task]; return n; });
  const onFiles = (jid, list) => setFiles(p => ({...p, [jid]: list}));
  const completeJob = async (job) => { console.log("Complete job", job, checked[job.id], files[job.id]); setDone(d=>({...d,[job.id]:true})); };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="p-5 rounded-2xl bg-emerald-50 border">
          ...
          <div className="mt-3 flex items-center gap-3">
            {!clockIn ? (
              ...
              <>
                <span className="text-sm">Clocked in at <strong>{new Date(clockIn).toLocaleTimeString([], {hour:"2-digit", minute:"2-digit"})}</strong></span>
        ...
        <div className="p-5 rounded-2xl bg-white border">
          ...
          <p className="text-xs text-slate-500 mt-2">v10 • {isoToday()}</p>
        </div>
      </div>
            ...
            <motion.div key={job.id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}
              ...
              <button onClick={()=>toggleJob(job.id)} className="w-full text-left">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h4 className="text-lg font-semibold text-slate-800">{titleLine(job)}</h4>
                    <div className="text-sm flex gap-2 mt-1 text-slate-700">
                      <Calendar className="w-4 h-4" />
                      <span>{subline(job)}</span>
                    </div>
                    {job.address && (
                      <div className="text-sm flex gap-2 mt-1 text-slate-700">
                  ...
                  {Object.entries(MASTER_CHECKLIST).map(([room, tasks])=>{
                    ...
                    return (
                      <div key={room} className="border rounded-xl">
                        <button type="button" onClick={()=>toggleRoom(job.id,room)} className="w-full flex items-center justify-between px-4 py-3">
...
// -------- CUSTOMER (simple placeholder so tab is real) --------
  ...
  return (
    <div className="p-6 rounded-2xl border bg-white">
      ...
      <p className="text-sm text-slate-600">
        Customer view is enabled and will show job photos & status tied to the customer’s phone.
        (We’ll wire this to your Supabase storage next.)
      </p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white">
      <header className="px-5 py-6 border-b">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="M.O.R. Clean Daytona" className="w-10 h-10 rounded-full border object-cover" />
